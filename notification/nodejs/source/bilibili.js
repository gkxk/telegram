...
async function get_at() {
	...
	return axios.get(url, {
		...
		items.forEach(item => {
			let id=`bili${item.id}`
			const author = item.user.nickname;
			const date = new Date(item.at_time * 1000).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' });
			let content= item.item.source_content;
			let link=item.item.uri
...
async function get_text() {
	let text = []
	let reply = await get_reply()
	if (reply) {
		text = text.concat(reply)
	}
	let at = await get_at()
	if (at) {
		text = text.concat(at)
